# 1/ Dire "Bonjour"
#   - prend 5 noms d'utilisateurs sur Twitter au hasard, et qui leur tweet une phrase, avec le hashtag #bonjour_monde
#   - envoi d'un tweet par journaliste (5 en tout)
#   - envoi également sur le compte @the_hacking_pro
# 2/ Liker "Bonjour"
...
# 4/ Liker et Follow en live
#   - mets en place un robot qui like et follow les comptes tweetant le hashtag #bonjour_monde dès l'apparition du Tweet
#   - tourne en tâche de fond
# 5/ Tests
#   - tester que la méthode de login
# hashtag : #mot-clé (dans un message)
...
# tweeter sur ton compte
# connection.update('#bonjour_monde !!!!')

# # 1/ envoi d'un tweet par journaliste (5 journalistes au hasard)
# journalists_to_tweet = 5.times.map{ journalists[rand(0...journalists.length)] }.uniq
# journalists_to_tweet.each { |chr| connection.update("Hello #{chr} #bonjour_monde @the_hacking_pro") }

# # 2/ liker les derniers tweets (min les 25 derniers) qui ont le hashtag #bonjour_monde
# connection.search("#bonjour_monde", result_type: "recent").take(25).collect do |tweet|
#     puts connection.fav tweet
# end

# # 3/ Follow les dernières personnes (min 20) qui ont tweeté avec le hashtag #bonjour_monde
# connection.search("#bonjour_monde", result_type: "recent").take(5).collect do |tweet|
#     if tweet.user != connection.user.id
#         #puts connection.follow tweet.user
#     end
# end

# # 4/ Robot qui like et follow les comptes tweetant le hashtag #bonjour_monde dès l'apparition du Tweet
# live.filter(track:"#bonjour_monde", result_type: "recent") do |tweet|
#     puts connection.favorite tweet
# end
